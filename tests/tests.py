...
class TestPagination(FlaskTestMixin):
    ...
    def test_defaults(self):
        ...
        with self.app.test_request_context('/'):
            ...
            assert pagination.current_page_fmt == \
                CURRENT_PAGES[pagination.css_framework]
            assert pagination.link_css_fmt == \
                CSS_LINKS[pagination.css_framework]
            assert pagination.gap_marker_fmt == \
                GAP_MARKERS[pagination.css_framework]
            assert pagination.prev_disabled_page_fmt == \
                PREV_DISABLED_PAGES[pagination.css_framework]
            assert pagination.next_disabled_page_fmt == \
                NEXT_DISABLED_PAGES[pagination.css_framework]
            assert pagination.prev_page_fmt == \
                PREV_PAGES[pagination.css_framework]
            assert pagination.next_page_fmt == \
                NEXT_PAGES[pagination.css_framework]
            assert pagination.css_end_fmt == \
